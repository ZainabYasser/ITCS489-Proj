...
function displayOrderDetails(order) {
    ...
    if (order.items && order.items.length > 0) {
        ...
        for (const item of order.items) {
            ...
            itemsHTML += `<li style="padding: 5px 0;">${escapeHtml(item.product_name)} x ${item.quantity} - ${priceFormatted}</li>`;
        }
        itemsHTML += '</ul></div>';
    }
    ...
    const paymentStatusColor = order.payment_status === 'completed' ? '#28a745' : '#ffc107';
    ...
    container.innerHTML = `
        ...
        <div class="order-number" style="font-size: 24px; font-weight: 700; color: #8B5E3C; background: #f9f6f3; display: inline-block; padding: 8px 20px; border-radius: 30px; margin: 20px 0;">#${escapeHtml(order.order_number)}</div>
        
        <div class="order-details" style="background: #f9f6f3; border-radius: 12px; padding: 20px; margin: 30px 0; text-align: left;">
            <div class="detail-row" style="display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid #e8e2d9;">
            ...
            <div class="detail-row" style="display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid #e8e2d9;">
                ...
                <span>${escapeHtml(order.shipping_address || 'N/A')}, ${escapeHtml(order.shipping_city || '')}</span>
            </div>
            ${itemsHTML}
        </div>
        ...
        <div class="btn-group" style="display: flex; gap: 15px; justify-content: center; margin-top: 30px;">
            <a href="../AUTH/account.html" class="btn-primary" style="background: #8B5E3C; color: white; padding: 12px 24px; border-radius: 8px; text-decoration: none;">View My Orders</a>
            <a href="shop.html" class="btn-secondary" style="background: #f5f0eb; color: #8B5E3C; padding: 12px 24px; border-radius: 8px; text-decoration: none;">Continue Shopping</a>
        </div>
    `;
}
